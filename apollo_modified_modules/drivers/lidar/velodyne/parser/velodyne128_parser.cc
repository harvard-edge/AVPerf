...
void Velodyne128Parser::GeneratePointcloud(
  ...
  double latency = cyber::Time().Now().ToSecond() - t_prev;
  if( latency < (1.0 / downsample_factor) ){
          AINFO << std::setprecision(16) << "Skip: " << cyber::Time().Now().ToSecond();
          return;
  }
  t_prev = cyber::Time().Now().ToSecond();
  AINFO << "DEBUG Parser: " << t_prev;
