...
void PredictorManager::Run(
  ...
  AINFO << "DEBUG predict time: " << (float) (clock() - predict_start) / CLOCKS_PER_SEC;
}
...
void PredictorManager::PredictObstacle(
  ...
  if (obstacle->ToIgnore()) {
    ...
    AINFO << "DEBUG obstacle_type: " << obstacle->type();
    AINFO << "DEBUG vehicle: " << PerceptionObstacle::VEHICLE;
...
void PredictorManager::RunVehiclePredictor(
  ...
  if (obstacle->IsCaution()) {
    if (obstacle->IsNearJunction()) {
      ...
      AINFO << "DEBUG predictor: vehicle_in_junction_caution_predictor";
    } else if (obstacle->IsOnLane()) {
      predictor = GetPredictor(vehicle_on_lane_caution_predictor_);
      AINFO << "DEBUG predictor: vehicle_on_lane_caution_predictor";
    } else {
      predictor = GetPredictor(vehicle_default_caution_predictor_);
      AINFO << "DEBUG predictor: vehicle_default_caution_predictor";
    }
  ...
  if (!obstacle->IsOnLane()) {
    ...
    AINFO << "DEBUG predictor: vehicle_off_lane_predictor";
    ...
    AINFO << "DEBUG predictor: vehicle_in_junction_predictor";
  } else {
    predictor = GetPredictor(vehicle_on_lane_predictor_);
    AINFO << "DEBUG predictor: vehicle_on_lane_predictor";
  }
