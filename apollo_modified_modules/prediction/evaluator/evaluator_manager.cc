...
Evaluator* EvaluatorManager::GetEvaluator(
  ...
  AINFO << "Run obstacle evaluator [" << type << "]";
...
void EvaluatorManager::Run(
  ...
  AINFO << "DEBUG evaluator time: " << (float) (clock() - evaluator_start) / CLOCKS_PER_SEC;
}
...
void EvaluatorManager::EvaluateObstacle(
  ...
  AINFO << "DEBUG evaluator: " << obstacle->type();
