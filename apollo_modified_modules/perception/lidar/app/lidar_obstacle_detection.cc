...
LidarProcessResult LidarObstacleDetection::Process(
  ...
  if( latency < (1.0 / downsample_factor) ){
	  return LidarProcessResult(LidarErrorCode::Succeed);
  }
  t_prev = message->measurement_time();
  AINFO << "DEBUG latency2: " << latency;
  time_de = Clock::NowInSeconds();
  AINFO << std::setprecision(16) << "DEBUG latency current time: "<< time_de;*/
  AINFO << "in pc processing";
