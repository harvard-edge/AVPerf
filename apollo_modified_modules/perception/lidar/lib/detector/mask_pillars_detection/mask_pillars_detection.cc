...
bool MaskPillarsDetection::Detect(const LidarDetectorOptions& options,
                                   LidarFrame* frame) {
  // double downsample_factor = 30.0 / FLAGS_downsample_beams_factor
  int downsample_factor = FLAGS_downsample_beams_factor;
  pc_num++;
  /*if( pc_num %downsample_factor > 0 ){
  	  return false;
  }*/
  
  double f_latency = frame->timestamp - frame_t_prev;
  //double latency = (double) (clock() - t_prev) / CLOCKS_PER_SEC;
  double latency = (double) (clock() - t_prev) / CLOCKS_PER_SEC;
  //float latency = frame->timestamp - frame_t_prev;
  /*if( f_latency < (1.0 / downsample_factor) ){
	  return false;
  }*/
  AINFO << "DEBUG T1: " << std::setprecision(16) << Clock::NowInSeconds();
  AINFO << "FPS: " << downsample_factor << ". point cloud number:  " << pc_num <<  ", latency: " << latency << std::setprecision(16) << ", f_latnecy: " << f_latency << ", frame timestamp: " << std::setprecision(20) << frame->timestamp;
  frame_t_prev = frame->timestamp;
  t_prev = clock();
  // check input
  if (frame == nullptr) {
  ...
  if (FLAGS_enable_downsample_beams) {
    ...
    if (DownSamplePointCloudBeams(original_cloud_, downsample_beams_cloud_ptr,
      ...
      AINFO << "downsample beam factor:  " << FLAGS_downsample_beams_factor;
    } else {
  ...
  if (FLAGS_enable_downsample_pointcloud) {
    ...
    AINFO << "downsample point cloud :  " << FLAGS_downsample_voxel_size_x;
  ...
  AINFO << "DEBUG inference time: " << (float) (clock() - inference_start) / CLOCKS_PER_SEC;
