...
bool DetectionComponent::Proc(
  ...
  double latency = message->measurement_time() - t_prev;
  if( latency < (1.0 / downsample_factor * 0.9) ){
  	  AINFO << std::setprecision(16) << "Skip: " << Clock::NowInSeconds();
          //return true;
          return false;
  }
  t_prev = message->measurement_time();
  ...
  AINFO << std::setprecision(16) << "DEBUG latency current time: "<< Clock::NowInSeconds();
  auto out_message = std::make_shared<LidarFrameMessage>();
  AINFO << "DEBUG latency: " << latency << ", pc_num: " << pc_num;
