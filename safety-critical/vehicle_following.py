...
npcState.transform.position = egoState.position + 30 * forward  # NPC is 3.6m to the left of the EGO
npc = sim.add_agent("Jeep", lgsvl.AgentType.NPC, npcState)

#npc2State = lgsvl.AgentState()
#npc2State.transform = egoState.transform
#npc2State.transform.position = egoState.position + 30 * forward- 3.6*right  # NPC is 3.6m to the left of the EGO
#npc2 = sim.add_random_agents("Sedan", lgsvl.AgentType.NPC, npc2State)
...
while True:
    ...
    if time.time() - t0 < 10:
        npc.follow_closest_lane(follow=True, max_speed=7.5, isLaneChange=False)
    elif time.time() - t0 < 20:
        npc.follow_closest_lane(follow=True, max_speed=5, isLaneChange=False)
    else:
        npc.follow_closest_lane(follow=True, max_speed=15, isLaneChange=False)

    sim.run(1, fraction)
    
    # Simulation will be limited to running for 40 seconds total
